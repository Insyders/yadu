    ...
    if (content[key] && typeof content[key] === 'object' && Object.keys(content[key]).length > 0) {
      Object.keys(content[key]).forEach((subKey) => {
        if (subKey === 'Ref' || subKey === 'Fn::Sub' || subKey === 'Fn::ImportValue') {
          // FIXME: if subkey is a string or number !
  ...
  ExtractLambdaInfo() {
    this.converted = this.lambdas.map((lambda) => {
      ...
      const lambdaExtracted = {
        ...
        Code: `${path.resolve(this.config.lambdaBasePath, lambda.Properties.CodeUri.split('/').reverse()[0])}index.zip`,
...
// {
//     "FunctionName": "a1-eb-accepted-offer-spruce",
//     "Description": "Spruce Adapter for acceptedOffer event",
//     "Code": "./a1-eb-accepted-offer-spruce/index.zip",
...
//     "Timeout": 300,
//     "Layers": [
//       "arn:aws:lambda:us-east-1:174831275467:layer:node_modules:53",
//       "arn:aws:lambda:us-east-1:174831275467:layer:eventbus:2"
//     ],
