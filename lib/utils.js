...
function defaultZip(currentWorkingDirectory) {
  console.log('Will try to determine the default zip args');
  const filesAndDirs = fs.readdirSync(currentWorkingDirectory);

  let fileToZip = filesAndDirs;

  if (filesAndDirs.includes('.npmignore')) {
    const npmIgnore = fs.readFileSync(`${currentWorkingDirectory}/.npmignore`, { encoding: 'utf-8' }).split('\n');

    fileToZip = fileToZip.filter((file) => {
      if (fs.statSync(`${currentWorkingDirectory}/${file}`).isDirectory()) {
        file += path.sep;
      }

      const found = !npmIgnore.some((ignored) => {
        // Skip empty line
        if (ignored !== '') {
          const regex = new RegExp(ignored.replace(/\*|\*\*/g, '.*'));
    ...
    console.log(`Found : ${fileToZip.join(' ')}`);
    return fileToZip.join(' ');
  }
  console.debug('Will package the whole directory. Please add a .npmignore to ignore development dependencies.');
}
