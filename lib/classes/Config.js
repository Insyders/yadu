...
class Config {
  constructor(config) {
    ...
    this.config.rds = config.rds || {};
  }

  static LoadConfig(configPath) {
    console.log('[Action] Load YaDU Configuration');
    ...
    try {
      ...
      if (process.env.env && process.env.env !== '') {
        ...
        console.error(
          `${
            '[WARN]'.warn
          } No Configuration loaded, you might want to specify --env=<filename> where the filename is one of the ./.yadu/ file.`,
        );
      }
      return null;
    }
