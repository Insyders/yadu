...
const ignore_patterns = ['.aws-sam', 'node_modules', '.git'];
module.exports = class Cloudformation {
  ...
  ScanDirectory(baseDir = null, parent = '.', depth = 0) {
    ...
    for (const dir of directories) {
      ...
      if (rgx.exec(dir)) {
        ...
        console.log(`${'SUCCESS'.success} Valid Template path : ${path.resolve(parent, baseDir)}${path.sep}${dir}`);
  ...
  ReadApplicationTemplate() {
    console.log('ReadApplicationTemplate');
    const convert = new Converter({
      lambdaBasePath: this.lambdaBasePath,
      stage: this.stage,
      accountId: this.accountId,
  ...
  SaveMergedTemplate(filename) {
    console.log('SaveMergedTemplate');
