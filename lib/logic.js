...
async function publish(args, config = {}) {
  ...
  if (info && info.zipArgs && info.zipArgs.includes('node_modules')) {
    ...
    throw new Error(
      // eslint-disable-next-line max-len
      `Not using NodeJS, no other runtime implemented yet. It is triggered because the directory \`node_modules/\` is not specified in your YaDU configuration \`zipArgs\` or not using the --zip-args hasn't been loaded correctly.`,
    );
  }
