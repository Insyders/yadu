...
async function publish(args, config = {}) {
  ...
  const ACCOUNT_ID = process.env.ACCOUNT_ID || getAccountId(args, sts);
  ...
  if (CheckIfServerlessExists() || args['use-yaml']) {
    let info = {};
    if (args['use-yaml']) {
      const readYaml = new Converter(config);
      console.log('todo...');
      readYaml.LoadYaml(config.applicationYaml);
      readYaml.ExtractLambdaInfo();
      // eslint-disable-next-line prefer-destructuring
      info = readYaml.GetConverted().filter((converted) => converted.codeUri.includes(process.cwd().split(path.sep).reverse()[0]))[0];
