...
function loadConfig(config) {
  ...
  logDebug(config.applicationYaml);
  logDebug(`Using : ${path.join(isHome(process.cwd()).path, config.applicationYaml)}`);
  readYaml.LoadYaml(path.join(isHome(process.cwd()).path, config.applicationYaml));
  readYaml.ExtractLambdaInfo();
  logVerbose(readYaml.GetConverted());

  // we use the process.cwd() because this command should be launch in the lambda directory
  ...
  const info = readYaml.GetConverted().filter((converted) => {
    logDebug(converted.Code);
    // Forced to use slashes because cloudformation is written using the /
    logDebug(`Is '${converted.Code}' includes '/${configPath}/' ?`);
    return converted.Code.includes(`/${configPath}/`);
  })[0];

  if (!info) {
    throw new Error(`Configuration '${configPath}' not Found using '${path.join(process.cwd(), config.applicationYaml)}'`);
  }
...
async function publish(args, config = {}) {
  ...
  if (args.deploy) {
    ...
    if (args.deploy) {
      console.log('Deploy the lambda configuration');
    ...
    if (args.create) {
      console.log('Create the lambda function');
