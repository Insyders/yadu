...
async function publish(args, config = {}) {
  ...
  if (args['use-yaml']) {
    ...
    logDebug(path.sep + process.cwd().split(path.sep).reverse()[0] + path.sep);

    // eslint-disable-next-line prefer-destructuring
    info = readYaml.GetConverted().filter((converted) => {
      logDebug(converted.Code);
      return converted.Code.includes(path.sep + process.cwd().split(path.sep).reverse()[0] + path.sep);
    })[0];

    if (!info) {
      throw new Error(
        `Configuration '${process.cwd().split(path.sep).reverse()[0]}' not Found using '${path.join(
          process.cwd(),
          config.applicationYaml,
        )}'`,
      );
    }
