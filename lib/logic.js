...
function loadConfig(config) {
  ...
  logDebug(`Application YAML Path: ${config.applicationYaml}`);
  if (!config || !config.applicationYaml) {
  ...
  const info = readYaml.GetConverted().filter((converted) => {
    ...
    return converted.Code.includes(`${path.sep}${configPath}${path.sep}`);
  })[0];

  if (!info) {
    throw new Error(`Configuration '${configPath}' not Found using '${path.join(isHome(process.cwd()).path, config.applicationYaml)}'`);
  }
...
async function publish(args, config = {}) {
  ...
  if (info.Layers && info.Layers.length > 0) {
    ...
    if (args['show-config']) {
      ...
      console.log(updateCreateFnParams);
      process.exit(0);
    }
